...
public interface TeacherRepository extends PagingAndSortingRepository<Teacher, Integer>{
    ...
    @Query("SELECT t FROM Teacher t JOIN t.groups g WHERE g.id = :groupId AND " +
            "(LOWER(t.name) LIKE :filter OR LOWER(t.date) LIKE :filter)")
    Page<Teacher> getTeacherInGroup(@Param("groupId") int groupId,
                                     @Param("filter") String filter,
                                     Pageable pageable);
    @Query("SELECT COUNT(t) FROM Teacher t JOIN t.groups g WHERE g.id = :groupId")
    int getCountTeacherInGroup(@Param("groupId") int groupId);

    @Query("SELECT t FROM Teacher t WHERE (LOWER(t.name) LIKE :filter " +
            "OR LOWER(t.date) LIKE :filter) " +
            "AND t NOT IN (SELECT t FROM Group g JOIN g.teachers t WHERE g.id = :groupId)")
